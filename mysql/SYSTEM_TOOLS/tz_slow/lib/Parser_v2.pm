######################################################################
package SQL::Parser_v2;
######################################################################
#
# This module is copyright (c), 2001,2005 by Jeff Zucker.
# This module is patched and focus on slow parse, 2015 by Keithlan.
# All rights resered.
#
...
sub TRUNCATE
{
    ...
    my $table_name;
    $self->{struct}->{command} = 'TRUNCATE';
    if ( $stmt =~ /^\s*TRUNCATE\s+TABLE\s+(.*)$/si )
    {
        $stmt = "TRUNCATE TABLE $1";
        $self->{struct}->{ignore_missing_table} = 1;
    }
    if ( $stmt =~ /^\s*TRUNCATE\s+(\S+)\s+(.+)$/si )
    {
        my $com2 = $1 || '';
        $table_name = $2;
        if ( $com2 !~ /^TABLE$/i )
        {
            return $self->do_err("The command 'TRUNCATE $com2' is not recognized or not supported!");
        }
        $table_name =~ s/^\s+//;
        $table_name =~ s/\s+$//;
        if ( $table_name =~ /(\S+) (RESTRICT|CASCADE)/i )
        {
            $table_name = $1;
            $self->{struct}->{truncate_behavior} = uc $2;
        }
    }
    else
    {
        return $self->do_err("Incomplete TRUNCATE statement!");
...
sub INSERT
{
    ...
    $str =~ s/\s+ON duplicate KEY.*//i; # add by keith , remove 'ON duplicate KEY'
...
sub REPLACE
{
    my ( $self, $str ) = @_;
    my $col_str;
    $str =~ s/^REPLACE\s+INTO\s+/REPLACE /i;    # allow INTO to be optional
    my ( $table_name, $val_str ) = $str =~ m/^REPLACE\s+(.+?)\s+VALUES\s+(\(.+?\))$/i;
    if ( $table_name and $table_name =~ m/[()]/ )
    {
        ( $table_name, $col_str, $val_str ) =
          $str =~ m/^REPLACE\s+(.+?)\s+\((.+?)\)\s+VALUES\s+(\(.+?\))$/i;
    }
    return $self->do_err('No table name specified!') unless ($table_name);
    return $self->do_err('Missing values list!')     unless ( defined $val_str );
    return undef                                     unless ( $self->TABLE_NAME($table_name) );
